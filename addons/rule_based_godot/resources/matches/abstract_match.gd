...
# TODO: remove or move the functions below
func _build_var_or_path(var_or_path: String) -> void:
	# Auxilary function
	if var_or_path.begins_with('?'):
		target_is_wildcard = true
		target_identifier = var_or_path.trim_prefix('?')
		target_path = ^""
	else:
		target_is_wildcard = false
		target_identifier = ""
		target_path = NodePath(var_or_path)

func _var_or_path_string() -> String:
	# Auxilary function
	return '?' + target_identifier if target_is_wildcard else var_to_str(target_path)

func _build_prop_or_method(json_repr: Array, start_index: int) -> void:
	# Auxilary function
	if json_repr.size() == start_index + 1: # only property
		extraction_type = 0
		extraction_property = json_repr[start_index]
		extraction_method = ""
		extraction_arguments = []
	else:
		extraction_type = 1
		extraction_property = ""
		extraction_method = json_repr[start_index]
		extraction_arguments = _eval_arguments(json_repr[start_index + 1])

func _prop_or_method_array() -> Array:
	# Auxilary function
	return [extraction_property] \
		if extraction_type == 0 \
		else [extraction_method, extraction_arguments]
